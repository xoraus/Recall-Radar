...
function SidebarWidget() {
  ...
  if (!stats || !settings) {
    return <div className="wct-loading wct-small">Loading\u2026</div>;
  }
  ...
  return (
    <div className="wct-sidebar">
      <h4>Wrong Card Tracker</h4>
      <div className="wct-sidebar-stats">
        <div>
      ...
      <h5>Top Forgotten</h5>
      <ul className="wct-sidebar-list">
        {worst.map((s) => (
          <li key={s.cardId} title={s.promptText}>
            {s.promptText.slice(0, 40) || '(empty)'} <span className="wct-muted">&times;{s.misses}</span>
          </li>
        ))}
        {worst.length === 0 && <li className="wct-muted">No missed cards yet.</li>}
      </ul>

      <button className="wct-primary-btn" onClick={() => plugin.window.openWidgetInPane('practice')}>
        Practice Now
      </button>
